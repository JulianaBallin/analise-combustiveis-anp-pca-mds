...
\begin{titlepage}
  ...
  \begin{tabularx}{\textwidth}{@{}L{4cm} Y@{}}
    ...
    \textbf{Data:}        & 26 de maio de 2026 \\[2mm]
...
\begin{itemize}[leftmargin=1.2cm, topsep=2pt, itemsep=2pt]
  ...
  \item \textbf{v2.0} (este documento): Parte 3 - Aplicação da primeira ferramenta AutoML (FLAML).
  \item \textbf{v3.0} (entrega final): Parte 4 - Aplicação da segunda ferramenta (LightAutoML); Parte 5 - Reflexão e comparação.
\end{itemize}
\end{infobox}
...
biblioteca de AutoML desenvolvida para encontrar bons modelos com baixo custo
computacional. A ferramenta automatiza a seleção de algoritmos, a busca de
hiperparâmetros e a avaliação de candidatos dentro de um orçamento de tempo definido.

No experimento da equipe, a FLAML foi utilizada para um problema de \textbf{regressão},
com o objetivo de prever a variável \texttt{preco\_medio\_gasolina\_c}, que representa
o preço médio mensal da gasolina C por UF.

\begin{infobox}
\textbf{Objetivo da Parte 3:} aplicar a primeira ferramenta AutoML atribuída à equipe,
registrar o processo utilizado, apresentar os modelos avaliados automaticamente e
interpretar o melhor resultado obtido no conjunto de teste.
\end{infobox}
...
A execução foi feita em ambiente virtual Python local no próprio projeto. As
dependências da primeira ferramenta AutoML foram adicionadas ao arquivo
\texttt{requirements.txt}, incluindo \texttt{flaml} e \texttt{lightgbm}.
...
divisão treino e teste definida na Parte 2, treina a FLAML, salva métricas e gera
gráficos de apoio.
...
A divisão usada foi de 80\% para treino e 20\% para teste, estratificada por região
geográfica e com \texttt{random\_state = 42}. Isso gerou 1.295 registros de treino e
324 registros de teste.

Para evitar vazamento de dados, algumas colunas foram removidas da modelagem
supervisionada:

\begin{itemize}[leftmargin=1.5cm, itemsep=4pt]
  \item \texttt{preco\_medio\_gasolina\_c}: variável-alvo.
  \item \texttt{variacao\_preco\_gasolina\_c}: derivada do próprio preço da gasolina C.
  \item \texttt{preco\_relativo\_etanol\_gasolina}: usa o preço da gasolina C no denominador.
  \item \texttt{mes\_ano}: substituída pelas colunas numéricas \texttt{ano} e \texttt{mes}.
  \item \texttt{uf\_nome}: informação duplicada da sigla \texttt{uf}.
\end{itemize}
...
\begin{tabularx}{\textwidth}{@{}L{5.2cm} Y@{}}
  ...
  \taltrow Estratificação & Região geográfica \\
...
\begin{enumerate}[leftmargin=1.5cm, itemsep=4pt]
  ...
  \item Remoção de colunas que causariam vazamento de dados.
  ...
  \item Configuração da FLAML para tarefa de regressão, métrica RMSE e orçamento de 60 segundos.
  \item Treinamento automático dos modelos candidatos.
  \item Avaliação do melhor modelo no conjunto de teste.
  \item Geração de métricas, leaderboard, previsões e gráficos.
\end{enumerate}
...
\begin{tabularx}{\textwidth}{@{}L{3cm} C{4cm} C{3cm} Y@{}}
  ...
  \texttt{extra\_tree} & 0,1287 & Não & Resultado próximo ao melhor modelo, usando árvores extremamente aleatórias. \\
...
\texttt{colsample\_bytree = 0,5934}.
...
\begin{tabularx}{0.9\textwidth}{@{}L{4.5cm} C{4cm} Y@{}}
  ...
  \midrule
  \taltrow RMSE & 0,1148 & Erro médio quadrático em R\$/litro, penalizando erros maiores. \\
...
O tempo aproximado de execução foi de \textbf{60,78 segundos}. Esse resultado mostra
que a FLAML conseguiu construir um modelo competitivo em pouco tempo, com configuração
manual mínima por parte da equipe.

\begin{figure}[H]
  \centering
  \IfFileExists{../../../outputs/figures/flaml_real_vs_previsto.png}{%
    \includegraphics[width=0.78\textwidth]{../../../outputs/figures/flaml_real_vs_previsto.png}
  }{%
...
variação de volume da gasolina C e o volume de gasolina C. Essa leitura é coerente
com o contexto do problema, pois o preço da gasolina C tende a se relacionar com o
preço do etanol, com o volume vendido e com padrões temporais e regionais.

\begin{figure}[H]
  \centering
  \IfFileExists{../../../outputs/figures/flaml_importancia_atributos.png}{%
    \includegraphics[width=0.86\textwidth]{../../../outputs/figures/flaml_importancia_atributos.png}
  }{%
...
% ------------------------------------------------------------------------------
\subsection{Interpretação e limitações da primeira aplicação}
\label{sec:flaml-interpretacao}
% ------------------------------------------------------------------------------

A FLAML apresentou bom desempenho na tarefa de regressão, com RMSE de 0,1148 no
conjunto de teste. Em termos práticos, o erro médio ficou em uma escala pequena diante
dos preços observados no dataset, que variam aproximadamente entre R\$ 4,25 e
R\$ 8,12 por litro.

Ao mesmo tempo, a interpretação deve ser feita com cuidado. O modelo aprende padrões
estatísticos do período analisado, mas não prova causalidade entre preço, volume,
região e tempo. Além disso, o experimento usa variáveis do mesmo mês da observação,
logo o resultado representa uma tarefa de modelagem supervisionada sobre dados
tabulares históricos, e não necessariamente uma previsão futura em cenário operacional.

\begin{notebox}
\textbf{Síntese da Parte 3:} a FLAML foi fácil de configurar, executou a busca
automática dentro do orçamento de tempo definido, selecionou \texttt{lgbm} como melhor
modelo e gerou métricas, leaderboard, previsões e importância de atributos. A etapa
humana continuou necessária para definir o alvo, remover colunas com vazamento de
dados, escolher a métrica principal e interpretar os resultados.
\end{notebox}
